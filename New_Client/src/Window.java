...
//-----------客户端主线程运行时，可以随时捕捉按钮动作，并将信息传入消息队列---------//
	public Window() {
		...
		button_connect.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){
				...
				try {
					...
					MessageReceiveThread messageReceiveThread = new MessageReceiveThread(socket, state, messageSendThread, debug,temp_frame,messageQueue);
					receiveThread = new Thread(messageReceiveThread);
					receiveThread.start();
